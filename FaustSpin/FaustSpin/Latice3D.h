...
namespace Lattice3D {
	...
	private:
		...
		std::vector<Cell> cells;// Vector to store the cells
	...
	inline Latice3D_Data create_lattice3D(const Latice3D_Data& ld, std::function<Cell(const Latice3D_Data&, int64_t, int64_t, int64_t)> func) {
		std::vector<Cell> new_cells(ld.get_N());
		size_t ion_count = 0;
...
namespace Lattice3D_Setters {
	inline Cell set_cube_cell(const Lattice3D::Latice3D_Data& lattice,int64_t x, int64_t y, int64_t z) {
		Cell cell_0 = lattice.get_cell0();
		int64_t x0 = cell_0.x;
		int64_t y0 = cell_0.y;
		int64_t z0 = cell_0.z;
		...
		int64_t y_cell = (y0 + y);
		int64_t z_cell = (z0 + z);
		return Cell(x_cell, y_cell, z_cell);
	}
}
...
namespace Lattice2D_Neighbors_Open_BC {
	...
	struct NeighborListCSR {
		std::vector<uint64_t> neighbors; // все соседи всех узлов подр€д, без вложенности
		std::vector<size_t> offsets;     // offsets[i]..offsets[i+1] Ч диапазон дл€ узла i, размер N+1
	};
	...
	// доступ к сосед€м узла i без копировани€: pair<указатель на первого соседа, число соседей>
